	...
	do
	{
		...
		for(iSec=0;iSec<(pInit_Args->SectorsPerClust);iSec++)
		{
			...
			for(iFDI=0;iFDI<NFDI_PER_SEC;iFDI++)
			{
				...
				if((pitem->Name[0]!=0xE5) && (CHK_ATTR_FILE(pitem->Attributes)) && ('.'!=pitem->Name[0]))//判断是不是文件被删除，排除.和..目录
				{
					*num=*num+1;
				}else if(pitem->Name[0]==0x00)
				{
					break;    //如果检测到是空的就不在查了，跳出来，查询完毕
				}
			}
		}
...
UINT8 znFAT_Modify_Name(struct FileInfo *pfi,INT8 *old_name_path,INT8 *new_name,UINT32 n,UINT8 is_file)
{
	...
	for(i=(UINT8)(fn_len-1);i>0;i--) //反向寻找. 第一个.是主文件与扩展名的分界
	{
	 ...
	 if(have_dot) //如果有点
	 {
		  ...
		  for(i=0;i<dot_pos;i++)
		  {
		   (pitem->Name)[i]=(INT8)Lower2Up(new_name[i]); //转为大写
		  }
		  for(;i<8;i++)
		  {
		  ...
		  for(i=(UINT8)(dot_pos+1);i<fn_len;i++)
		  {
		   (pitem->Extension)[j]=(UINT8)Lower2Up(new_name[i]); //转为大写
		   j++;
		  }
		  for(;j<3;j++)
		  {
		   (pitem->Extension)[j]=' '; //不足8字节部分填入空格
		  }
	 }
	  else //如果没有点
	 {
		  ...
		  for(i=0;i<fn_len;i++)
		  {
		   (pitem->Name)[i]=(UINT8)Lower2Up(new_name[i]); //转为大写
		  }
		  for(;i<8;i++)
		  {
		   (pitem->Name)[i]=' '; //不足8字节部分填入空格
